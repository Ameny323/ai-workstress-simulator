...
interface MatchingDescription {
  id: number;
  text: string;
}

interface MatchingInstanceData {
  items: MatchingItem[];
  descriptions: MatchingDescription[];
}

// Shape of the raw JSON returned by GET /sessions/{id}/next-task for an
// image_matching task (the backend's TaskOut schema).
export interface MatchingTaskData {
  id: string;
  type: string;
  title: string;
  description: string | null;
  instance_data: MatchingInstanceData;
...
interface MatchingTaskProps {
  task: MatchingTaskData;
  onSubmitted?: () => void;
}

// Same fallback table as ValidationTask/DocumentOrganizationTask, kept in
// sync deliberately — all task types share the same pressure-ramp-by-
// difficulty design.
...
function resolveDurationSeconds(task: MatchingTaskData): number {
...
export default function MatchingTask({ task, onSubmitted }: MatchingTaskProps) {
  const { items, descriptions } = task.instance_data;
  ...
  const match = (itemId: number, descriptionId: number) => {
    if (submitted) return;
    setMatches((prev) => ({ ...prev, [itemId]: descriptionId }));
  };
  ...
  // Same per-task countdown + auto-submit-on-timeout behavior as the
        ...
        {items.map((item) => (
          <div
            ...
            style={{
              ...
              background: matches[item.id] ? "rgba(91,132,198,0.04)" : "transparent",
          ...
          >
            ...
            <span style={{ flex: 1, fontSize: 12, color: "#94a3b8" }}>Item #{item.id}</span>
            <select
              value={matches[item.id] ?? ""}
              onChange={(e) => match(item.id, Number(e.target.value))}
              disabled={submitted}
              style={{
                padding: "6px 10px",
                borderRadius: 7,
                ...
                fontSize: 12.5,
                color: matches[item.id] ? "#1A2B3C" : "#94a3b8",
                background: "white",
        ...
        <span style={{ fontSize: 12, color: submitError ? "#c0505a" : "#94a3b8" }}>
          {submitError
            ...
            : submitted
              ...
              : `${items.length} images, ${matchedCount} matched so far.`}
        </span>
        <button
